...
| --- | --- | --- |
| Subfief/totem count | Repo-owned experimental knob `DUNE_SUBFIEF_LIMIT` or direct `DUNE_SUBFIEF_LIMIT_BONUS`; binary evidence shows `SubfiefCount`, `SubfiefLimitBonus`, and claim-capacity UI hooks; DB migrations include an empty `DUNE-94841_subfief_cap.sql`. | `scripts/apply-subfief-limit-knob.sh .env` writes `DunePlayerCharacterAttributeSet.SubfiefLimitBonus` into persisted player-character actors and current `dune.player_state.player_pawn_id` rows, including blank-class pawn rows. It installs triggers for future player actor saves and player-state pawn changes. `scripts/player-presence-announcer.py` also repairs joined/rejoined players whose current pawn bonus is below the configured value. `DUNE_SUBFIEF_LIMIT_BONUS` wins when set. Validate in game before calling this confirmed. |
