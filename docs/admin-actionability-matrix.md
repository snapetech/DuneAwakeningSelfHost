...
| Native offline player teleport | `POST /api/admin/player-recovery/offline-teleport` | `DUNE_ADMIN_MUTATIONS_ENABLED` + `DUNE_ADMIN_OFFLINE_TELEPORT_ENABLED` | `MOVE OFFLINE PLAYER` | high | Fingerprint-bound preview, dual Offline checks, finite coordinate bounds, full backup, advisory/row locks, native move, transform readback, private receipt, and isolated rolled-back proof. |
| Network-disconnect teleport | planned | planned network-disconnect gates | `NETWORK DISCONNECT TELEPORT` | high observed, moderate generalized | Verified on test player/Survival: real `UNetConnection` timeout, wait for `Offline`, `admin_move_offline_player_to_partition`, reconnect loaded moved pawn. See `docs/soft-disconnect-teleport.md`. |
