...
| Scheduled backup and retention | Icehunter, Reditus, Manaiakalani | DASH exceeds | Manual/scheduled paths share coverage-checked enabled-store snapshots; host/container locking prevents assured-deployment races; bounded verification and short failure retries retain exact diagnostics; successful schedule-owned sets alone enter retention; metrics, alerts, and the signed briefing expose reliability; see `docs/automatic-backups.md` |
...
| Signed change-aware operator briefing | Surveyed peers expose dashboards, logs, health cards, or bots but no retained cross-system signed decision queue | DASH exceeds | Seventeen authoritative readiness/governance/SLO/drift/deployment/assured-backup/automatic-backup/calendar/on-call-alert/recovery/capacity/credential/proof/update verdicts compile into a deterministic score, severity-ranked action queue, categorical regression/improvement deltas, current-input/age verification, private HMAC receipts, Overview/API, verified mixed backups, readiness, label-free metrics, and alerts; recommendations never execute; see `docs/operations-briefing.md` |
...
| Cross-scheduler conflict detection and runtime serialization | No surveyed Dune peer | DASH exceeds | Automatic backups, executing maintenance, recurring events, prewarming, and SLO exclusions share a bounded UTC calendar with deterministic critical/warning overlaps, complete maintenance-coverage findings, admission refusal, explicit audited override, label-free metrics/alerts, and full disruptive-window deferral behind the backup/deployment operation lock; see `docs/operations-calendar.md` |
| Durable on-call alert inbox and transition delivery | Surveyed peers expose health cards, logs, Discord/status integrations, or Prometheus rules; no surveyed Dune peer retains an acknowledgement-aware authoritative alert state machine | DASH exceeds | Successful Prometheus polls alone create/advance/resolve bounded alert identities; failed polls resolve nothing; duplicate polls emit nothing; re-fires form new generations and clear old ownership; operator acknowledgement is attributed without silencing; transition-only audit events reuse signed webhook delivery; private SQLite history, verified backups, API/UI/readiness, label-free metrics, collector/worker meta-alerts, and signed briefing integration close the response loop; see `docs/alert-inbox.md` |
