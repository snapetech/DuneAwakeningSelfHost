...
| Maintenance execution evidence | Restart documentation described scheduling and logs but did not prove that the new stopped-world backup was valid before update admission or retain a semantic execution outcome. | Added `docs/maintenance-intelligence.md`, mandatory pre-update backup verification, current-build recovery without false success, private signed stage receipts, Operations/API/metrics history, mixed-backup verification, tests, and assured-deployment closure. |
| Base pack-up cooldown | dapdsm v0.5.0 added totem cooldown inspection/reset after the aggregate peer audit, but DASH had no equivalent operator path or documented timestamp semantics. | Added `docs/base-packup-cooldown.md`, raw timestamp inventory, explicit unknown-countdown semantics, independent gate, stopped-map/offline-owner preview, backup-first compare-and-swap reset, readback verification, private receipts, UI/API, canary coverage, and guarded restart handoff. |
| Peer-audit drift | Aggregate parity pins could become stale without an operator revisiting every primary repository manually. | Added `docs/peer-watch.md` and a default-on, allowlisted, read-only primary-source watcher with per-peer failure isolation, retained transitions, Discovery/API status, briefing integration, readiness, backups, label-free metrics, and alerts. Pins remain review-only and are never updated automatically. |
