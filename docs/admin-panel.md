...
The host-side daily scheduler uses `scripts/schedule-daily-maintenance.sh`. Run it from cron or systemd before the desired maintenance time; the deployed schedule calls it at 02:30 and creates an executed, backed-up, announced `all` restart for 03:00. Its announcement cadence is 30 minutes of notice, every 5 minutes until 5 minutes remain, then every 1 minute through the final notice.
