...
| Updates/repair | Game/stack updates, schedule, runtime repair | Steam update flow, candidate-validated Git fast-forward, hotfix timer, reboot resume, image verification and post-start repair; certified maintenance revalidates the exact staged candidate before disruption and otherwise stays on the current build | DASH exceeds |
