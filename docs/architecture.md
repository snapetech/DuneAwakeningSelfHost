...
- Server-side 30-partition warm-pool registration is proven when status reports `current_alive_active=30 active_servers=30 partitions=30` with `COMPOSE_FILES='compose.yaml:compose.allmaps.yaml'`. Partition 31 / second Deep Desert is opt-in.
