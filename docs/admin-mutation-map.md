...
The chat-command implementation deliberately does not write live online actor
state. Online players are owned by the running map server, so a raw actor
transform update can be overwritten or desynced. A same-partition live test
moved the test player's controller, player-state, and pawn actor rows together
by `+750` X and incremented their serial; the live Survival server then wrote
the old in-memory position back on the next save cycle. `&teleport
<playername>` now resolves the admin and target, rejects online targets, and
routes preview plus optional execution through the loopback Admin endpoint.
There is no remaining direct chat-to-database teleport write: backup,
fingerprint, locks, native call, readback, approval, audit, and receipt policy
are identical to the dashboard path.
