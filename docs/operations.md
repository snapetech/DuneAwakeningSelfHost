...
The Ops tab also has a restart-announcement scheduler. It stores scheduled jobs in `backups/admin-panel/announcements.json` and invokes `DUNE_ADMIN_ANNOUNCE_COMMAND` at the chosen repeat interval until the scheduled restart time. The default hook, `scripts/announce.sh`, publishes a configurable JSON-RPC `ServiceBroadcast` envelope to the admin RabbitMQ `rpc` exchange. The RabbitMQ user must use a trusted service identity such as `bgd.<world-unique-name>.duneadmin.admin`; the local auth shim grants that identity management access and the game accepts the matching AMQP `user_id`. `DUNE_ANNOUNCE_PAYLOAD_MODE` can switch among built-in probe envelopes while validating the live server's expected wrapper. Keep `DUNE_ANNOUNCE_RMQ_PASSWORD` secret.
