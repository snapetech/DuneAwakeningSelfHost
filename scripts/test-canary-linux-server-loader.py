...
class CanaryLinuxServerLoaderTests(unittest.TestCase):
    ...
    def test_live_restart_guard_rechecks_players_and_skips_cleanup_restart(self):
        ...
        self.assertIn('if require_zero_players "$phase" && require_partition_active "$phase"; then', guarded_restart_body)
    ...
    def test_partition_active_guard_blocks_ambiguous_runtime_canaries(self):
        self.assertIn('require_active="${DUNE_LINUX_SERVER_CANARY_REQUIRE_ACTIVE:-true}"', self.source)
        self.assertIn("DUNE_LINUX_SERVER_CANARY_REQUIRE_ACTIVE default: true", self.source)
        self.assertIn("current_partition_runtime_state()", self.source)
        self.assertIn("require_partition_active()", self.source)
        state_body = self.function_body("current_partition_runtime_state")
        guard_body = self.function_body("require_partition_active")
        guarded_restart_body = self.function_body("restart_canary_if_zero_players")

        self.assertIn("dune.active_server_ids", state_body)
        self.assertIn("coalesce(fs.ready, false)::text", state_body)
        self.assertIn("coalesce(fs.alive, false)::text", state_body)
        self.assertIn("(asi.server_id is not null)::text", state_body)
        self.assertIn('case "$require_active"', guard_body)
        self.assertIn("partition_active_guard_${phase}_skipped=true", guard_body)
        self.assertIn("partition_guard_${phase}_ready=", guard_body)
        self.assertIn("refusing canary %s: partition %s ready=%s alive=%s active=%s", guard_body)
        self.assertIn('require_zero_players "$phase" && require_partition_active "$phase"', guarded_restart_body)
        self.assertIn("require_partition_active preflight", self.source)
    ...
    def test_preflight_only_exits_before_mutating_or_restarting(self):
        ...
        active_guard_index = self.source.index("require_partition_active preflight")
        ...
        self.assertLess(active_guard_index, build_index)
