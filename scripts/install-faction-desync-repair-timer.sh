...
usage() {
  cat >&2 <<'USAGE'
Usage: scripts/install-faction-desync-repair-timer.sh [ENV_FILE] [SERVICE_PATH] [TIMER_PATH]

Installs a systemd timer that repairs safe Dune faction desyncs from controller
faction JSON into normalized faction tables. Unsafe guild-conflict cases are
reported but skipped by the repair script.
USAGE
}
...
env_file="${1:-.env}"
service_path="${2:-/etc/systemd/system/dune-faction-desync-repair.service}"
timer_path="${3:-/etc/systemd/system/dune-faction-desync-repair.timer}"
repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
service_template="$repo_root/config/systemd/dune-faction-desync-repair.service"
timer_template="$repo_root/config/systemd/dune-faction-desync-repair.timer"
...
if [[ ! -f "$service_template" || ! -f "$timer_template" ]]; then
  printf 'faction desync repair systemd templates are missing\n' >&2
  exit 1
fi
...
sed \
  ...
  -e "s#^ExecStart=.*#ExecStart=$repo_root/scripts/audit-faction-desync.py --env-file $env_path --execute --confirm \"REPAIR FACTION DESYNC\" --required-host kspls0#" \
