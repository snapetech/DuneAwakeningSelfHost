...
write_gdb_cmd() {
  local pid="$1" base="$2" points_file="$3"
  {
    ...
    if [[ -n "$points_file" ]]; then
      ...
      done < <(brt_dd_trace_emit_points "$points_file" wide)
    else
      while read -r name off; do
        [[ -z "${name:-}" || "${name:0:1}" == "#" ]] && continue
        emit_bp "$base" "$name" "$off"
...
while true; do
  ...
  points_file="$(brt_dd_trace_points_or_stale_override "$pid" "brt-dd-wide-persistent-trace")"
  write_gdb_cmd "$pid" "$base" "$points_file"
  echo "$(ts) BRT_DD_PERSISTENT_TRACE_WIDE attaching pid=$pid base=$base points=${points_file:-builtins}" >>"$log"
