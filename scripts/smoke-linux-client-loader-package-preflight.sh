...
if not (packageClass and packageClass.Name=='DuneProbeSelfTestClass' and packageClass.ClassName=='UClass') then
  error('package LoadClass did not preserve registry class fallback')
end
if not (classBridge and classBridge.TargetName=='StaticLoadClass' and classBridge.Status=='anchor-missing' and not classBridge.NativeBridgeArmed) then
  error('LoadClass package bridge state did not block missing StaticLoadClass anchor')
end
if not (classAbi and classAbi.SignatureFamily=='StaticLoadClass' and classAbi.PlatformAbi=='sysv-x86_64' and not classAbi.AbiVerified and not classAbi.CallFrameReady) then
  error('LoadClass package ABI state did not expose StaticLoadClass contract')
end
if not (classFrame and classFrame.Status=='anchor-missing' and classFrame.BoundedInput and not classFrame.AbiVerified and not classFrame.ClassRootReady and not classFrame.CallFrameReady and not classFrame.NativeInvoked) then
  error('LoadClass package call frame state did not block missing anchor')
end
if not (classExecutor and classExecutor.ExecutorKind=='guarded-class-package-native-executor' and classExecutor.TargetName=='StaticLoadClass' and not classExecutor.NativeExecutorReady and classExecutor.NativeExecutorBlockReason=='anchor-missing' and not classExecutor.NativeInvoked) then
  error('LoadClass package executor state did not block missing anchor')
end
if not (classNative and classNative.Status=='anchor-missing' and classNative.TargetName=='StaticLoadClass' and classNative.InvokeEnabled==false and not classNative.NativeCallable and not classNative.NativeInvoked and not classNative.NativeCallPlanAccepted) then
  error('LoadClass package native invoke did not block missing anchor')
end
