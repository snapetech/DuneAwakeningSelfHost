...
usage() {
  cat >&2 <<'USAGE'
Usage: scripts/install-hotfix-auto-update-timer.sh [ENV_FILE] [SERVICE_PATH] [TIMER_PATH]

Installs the systemd timer that polls the owned Steam package for Dune hotfixes
and restarts the live farm only after a package/image update is loaded.
USAGE
}
...
env_file="${1:-.env}"
service_path="${2:-/etc/systemd/system/dune-hotfix-auto-update.service}"
timer_path="${3:-/etc/systemd/system/dune-hotfix-auto-update.timer}"
repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
service_template="$repo_root/config/systemd/dune-hotfix-auto-update.service"
timer_template="$repo_root/config/systemd/dune-hotfix-auto-update.timer"
...
if [[ ! -f "$service_template" || ! -f "$timer_template" ]]; then
  printf 'hotfix auto-update systemd templates are missing\n' >&2
  exit 1
fi
...
sed \
  ...
  -e "s#^ExecStart=.*#ExecStart=$repo_root/scripts/hotfix-auto-update-and-restart.sh $env_path#" \
