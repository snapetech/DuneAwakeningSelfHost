...
def summarize(records, exe_substring, pid):
    server_pids = set()
    loaded = []
    for record in records:
        if record.get("event") == "loaded":
            loaded.append(record)
            if exe_substring and exe_substring in record.get("exe", ""):
                server_pids.add(record.get("pid"))
    ...
    hits = [
        record
        for record in records
        if record.get("event") == "scan-hit"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    finishes = [
        record
        for record in records
        if record.get("event") == "scan-finish"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    skips = [
        record
        for record in records
        if record.get("event") == "scan-skip"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    hook_self_tests = [
        record
        for record in records
        if record.get("event") == "hook-dispatch-self-test"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    mod_self_tests = [
        record
        for record in records
        if record.get("event") == "mod-dispatch-self-test"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_self_tests = [
        record
        for record in records
        if record.get("event") == "lua-dispatch-self-test"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    lua_reflection_self_tests = [
        record
        for record in records
        if record.get("event") == "lua-reflection-self-test"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_process_event_self_tests = [
        record
        for record in records
        if record.get("event") == "lua-process-event-self-test"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    passed_lua_process_event_self_tests = [
        record for record in lua_process_event_self_tests if record.get("status") == "passed"
    ]
    ...
    lua_mod_scripts = [
        record
        for record in records
        if record.get("event") == "lua-mod-script"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_mod_dispatch_self_tests = [
        record
        for record in records
        if record.get("event") == "lua-mod-dispatch-self-test"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_mod_finishes = [
        record
        for record in records
        if record.get("event") == "lua-mod-finish"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    lua_load_asset_package_abi_state_events = [
        record
        for record in records
        if lua_load_asset_package_abi_state_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_load_asset_package_string_bridge_events = [
        record
        for record in records
        if lua_load_asset_package_string_bridge_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_load_asset_package_native_buffer_events = [
        record
        for record in records
        if lua_load_asset_package_native_buffer_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_load_asset_package_tchar_buffer_events = [
        record
        for record in records
        if lua_load_asset_package_tchar_buffer_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_load_asset_package_tchar_verification_events = [
        record
        for record in records
        if lua_load_asset_package_tchar_verification_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_load_asset_package_call_frame_events = [
        record
        for record in records
        if lua_load_asset_package_call_frame_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_load_asset_package_call_frame_verification_events = [
        record
        for record in records
        if lua_load_asset_package_call_frame_verification_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_load_asset_package_crash_guard_events = [
        record
        for record in records
        if lua_load_asset_package_crash_guard_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_load_asset_package_guarded_call_events = [
        record
        for record in records
        if lua_load_asset_package_guarded_call_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_load_asset_package_return_validation_events = [
        record
        for record in records
        if lua_load_asset_package_return_validation_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_load_asset_package_native_call_adapter_events = [
        record
        for record in records
        if lua_load_asset_package_native_call_adapter_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_load_asset_package_invocation_descriptor_events = [
        record
        for record in records
        if lua_load_asset_package_invocation_descriptor_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    lua_load_asset_package_native_executor_events = [
        record
        for record in records
        if lua_load_asset_package_native_executor_event_passed(record)
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    lua_function_iteration_checks = [
        record
        for record in records
        if record.get("event") == "lua-function-iteration-check"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    lua_process_event_params_buffers = [
        ...
        for record in records
        if (not server_pids or record.get("pid") in server_pids)
        and record.get("event") == "lua-process-event-params-buffer"
    ...
    ue_anchor_signatures = [
        record
        for record in records
        if record.get("event") == "ue-anchor-signature"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_pointers = [
        record
        for record in records
        if record.get("event") == "ue-pointer"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_layout_slots = [
        record
        for record in records
        if record.get("event") == "ue-layout-slot"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_uobjects = [
        record
        for record in records
        if record.get("event") == "ue-uobject"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_reflections = [
        record
        for record in records
        if record.get("event") == "ue-reflection"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_reflection_slots = [
        record
        for record in records
        if record.get("event") == "ue-reflection-slot"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_reflection_fields = [
        record
        for record in records
        if record.get("event") == "ue-reflection-field"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_reflection_properties = [
        record
        for record in records
        if record.get("event") == "ue-reflection-property"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_reflection_values = [
        record
        for record in records
        if record.get("event") == "ue-reflection-value"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_function_param_roots = [
        record
        for record in records
        if record.get("event") == "ue-function-param-root"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_function_params = [
        record
        for record in records
        if record.get("event") == "ue-function-param"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_function_param_container_children = [
        record
        for record in records
        if record.get("event") == "ue-function-param-container-child"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ue_function_native_identities = [
        record
        for record in records
        if record.get("event") == "ue-function-native-identity"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    lua_object_registry = [
        record
        for record in records
        if record.get("event") == "lua-object-registry"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    lua_object_registry_checks = [
        record
        for record in records
        if record.get("event") == "lua-object-registry-check"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    lua_function_registry_checks = [
        record
        for record in records
        if record.get("event") == "lua-function-registry-check"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    lua_object_outer_chains = [
        record
        for record in records
        if record.get("event") == "lua-object-outer-chain"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    lua_global_runtime_helper_checks = [
        record
        for record in records
        if record.get("event") == "lua-global-runtime-helper-check"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_object_array_shapes = [
        record
        for record in records
        if record.get("event") == "ue-object-array-shape"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_object_array_finishes = [
        record
        for record in records
        if record.get("event") == "ue-object-array-finish"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_object_array_items = [
        record
        for record in records
        if record.get("event") == "ue-object-array-item"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ue_object_native_identities = [
        record
        for record in records
        if record.get("event") == "ue-object-native-identity"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_fname_finishes = [
        record
        for record in records
        if record.get("event") == "ue-fname-finish"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_process_event_hooks = [
        record
        for record in records
        if record.get("event") == "ue-process-event-hook"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_call_function_hooks = [
        record
        for record in records
        if record.get("event") == "ue-call-function-hook"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_call_function_live_hooks = [
        record
        for record in records
        if record.get("event") == "ue-call-function-live-hook"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_process_event_live_hooks = [
        record
        for record in records
        if record.get("event") == "ue-process-event-live-hook"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_process_event_live_contexts = [
        record
        for record in records
        if record.get("event") == "ue-process-event-live-context"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_process_event_live_registry_contexts = [
        record
        for record in records
        if record.get("event") == "ue-process-event-live-registry-context"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_process_event_dispatch_self_tests = [
        record
        for record in records
        if record.get("event") == "ue-process-event-dispatch-self-test"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    ue_process_event_live_lua_dispatches = [
        record
        for record in records
        if record.get("event") == "ue-process-event-live-lua-dispatch"
        and (not server_pids or record.get("pid") in server_pids)
    ]
    ...
    maps_by_name = defaultdict(Counter)
    categories = Counter()

    for hit in hits + mapped_ue_anchors:
        name = hit.get("name", "")
        if not name:
            continue
        kinds_by_name[name][hit.get("kind") or hit.get("event", "")] += 1
        maps_by_name[name][hit.get("map", "")] += 1
    ...
    for name in sorted(offsets_by_name):
        ...
        by_name[name] = {
            ...
            "maps": dict(maps_by_name[name]),
            "first": offsets[0],
            "offsets": offsets,
        }

    return {
        "serverPids": sorted(pid for pid in server_pids if pid),
        "loaded": loaded,
        "scanFinish": finishes,
        "scanSkips": skips,
        "hookSelfTestCount": len(hook_self_tests),
        "passedHookSelfTestCount": sum(1 for record in hook_self_tests if record.get("status") == "passed"),
        "modSelfTestCount": len(mod_self_tests),
        "passedModSelfTestCount": sum(1 for record in mod_self_tests if record.get("status") == "passed"),
        ...
        "passedLuaReflectionSelfTestCount": sum(
            1 for record in lua_reflection_self_tests if lua_reflection_self_test_passed(record)
        ),
        "namedLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record) and record_int(record, "namedPropertyHits") > 0
        ),
        "rawSetLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record) and record_int(record, "rawPropertySetHits") > 0
        ),
        "numericLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_numeric_property_values_passed(record)
        ),
        "arrayInnerLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_array_inner_property_passed(record)
        ),
        "enumLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_enum_property_passed(record)
        ),
        "containerLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_container_property_passed(record)
        ),
        "importTextLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_import_text_passed(record)
        ),
        "exportTextLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_export_text_passed(record)
        ),
        "propertyMetadataLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_property_metadata_passed(record)
        ),
        "descriptorValueLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_descriptor_values_passed(record)
        ),
        "reflectionForEachPropertyLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_for_each_property_passed(record)
        ),
        "runtimeReflectionForEachPropertyLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "runtimeReflectionForEachPropertyCallbacks") > 0
        ),
        "selfTestReflectionForEachPropertyLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "selfTestReflectionForEachPropertyCallbacks") > 0
        ),
        "typedLiveDescriptorLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "liveDescriptorTypedClassHits") > 0
        ),
        "runtimeTypedLiveDescriptorLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "runtimeLiveDescriptorTypedClassHits") > 0
        ),
        "selfTestTypedLiveDescriptorLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "selfTestLiveDescriptorTypedClassHits") > 0
        ),
        "typedLiveDescriptorValueLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "liveDescriptorTypedValueHits") > 0
        ),
        "runtimeTypedLiveDescriptorValueLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "runtimeLiveDescriptorTypedValueHits") > 0
        ),
        "selfTestTypedLiveDescriptorValueLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "selfTestLiveDescriptorTypedValueHits") > 0
        ),
        "typedLiveDescriptorValueSetLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "liveDescriptorTypedValueSetHits") > 0
        ),
        "runtimeTypedLiveDescriptorValueSetLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "runtimeLiveDescriptorTypedValueSetHits") > 0
        ),
        "selfTestTypedLiveDescriptorValueSetLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "selfTestLiveDescriptorTypedValueSetHits") > 0
        ),
        "liveDescriptorValueLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_live_descriptor_values_passed(record)
        ),
        "runtimeLiveDescriptorValueLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "runtimeLiveDescriptorValueGetHits") > 0
        ),
        "selfTestLiveDescriptorValueLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
            if lua_reflection_self_test_passed(record)
            and record_int(record, "selfTestLiveDescriptorValueGetHits") > 0
        ),
        "nameTextLuaReflectionSelfTestCount": sum(
            1
            for record in lua_reflection_self_tests
        ...
        "passedLuaModScriptCount": sum(1 for record in lua_mod_scripts if record.get("status") == "passed"),
        "luaModDispatchSelfTestCount": len(lua_mod_dispatch_self_tests),
        "passedLuaModDispatchSelfTestCount": sum(1 for record in lua_mod_dispatch_self_tests if record.get("status") == "passed"),
...
def markdown(summary, top):
    lines = []
    lines.append("# Linux Loader Scan Summary")
    lines.append("")
    pids = ", ".join(summary["serverPids"]) or "all"
    lines.append(f"- Server PID filter: `{pids}`")
    lines.append(f"- Hit count: `{summary['hitCount']}`")
    lines.append(f"- Unique names: `{summary['uniqueHitCount']}`")
    for finish in summary["scanFinish"]:
        fields = " ".join(
            f"{key}={finish[key]}"
            for key in ("mappings", "scanned", "filtered", "unreadable", "sizeSkipped", "hits")
            if key in finish
        )
        lines.append(f"- Scan finish: `{fields}`")
    if summary["scanSkips"]:
        lines.append(f"- Scan skips: `{len(summary['scanSkips'])}`")
    ...
    if summary["ueProcessEventHookCount"]:
        lines.append(f"- UE ProcessEvent hook probes: `{summary['passedUeProcessEventHookCount']}/{summary['ueProcessEventHookCount']}` passed")
    if summary["ueCallFunctionHookCount"]:
        lines.append(f"- UE CallFunctionByNameWithArguments hook probes: `{summary['passedUeCallFunctionHookCount']}/{summary['ueCallFunctionHookCount']}` passed")
    ...
    if summary["hookSelfTestCount"]:
        lines.append(f"- Hook self-tests: `{summary['passedHookSelfTestCount']}/{summary['hookSelfTestCount']}` passed")
    if summary["modSelfTestCount"]:
        lines.append(f"- Mod self-tests: `{summary['passedModSelfTestCount']}/{summary['modSelfTestCount']}` passed")
    ...
    if summary["ueObjectArrayCount"]:
        lines.append(f"- UE object arrays: `{summary['finishedUeObjectArrayCount']}/{summary['ueObjectArrayCount']}` finished")
    ...
    if summary["ueFNameCount"]:
        lines.append(f"- UE FName decodes: `{summary['decodedUeFNameCount']}/{summary['ueFNameCount']}` decoded")
    lines.append("")
    ...
    lines.append("")
    items = sorted(
        summary["hitsByName"].items(),
        key=lambda item: (item[1]["category"], item[0]),
    )
    for name, data in items:
        offsets = ", ".join(offset["imageOffset"] for offset in data["offsets"][:top])
        if len(data["offsets"]) > top:
            offsets += f", ... +{len(data['offsets']) - top}"
        lines.append(
            f"- `{data['category']}` `{name}` count=`{data['count']}` first=`{data['first']['imageOffset']}` offsets=`{offsets}`"
        )
    lines.append("")
    return "\n".join(lines)
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Summarize Dune Linux server loader scan logs.")
    parser.add_argument("log", type=Path)
    parser.add_argument("--exe-substring", default="DuneSandboxServer")
    parser.add_argument("--pid", type=int)
    parser.add_argument("--format", choices=("json", "markdown"), default="markdown")
    parser.add_argument("--top", type=int, default=8, help="offsets per name in markdown output")
    args = parser.parse_args(argv)

    summary = summarize(load_records(args.log), args.exe_substring, args.pid)
    if args.format == "json":
