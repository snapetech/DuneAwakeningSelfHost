...
usage() {
  cat >&2 <<'USAGE'
Usage: scripts/install-target-safety-audit-timer.sh [ENV_FILE] [SERVICE_PATH] [TIMER_PATH]

Installs the read-only Dune target safety audit systemd timer for this checkout.
USAGE
}
...
env_file="${1:-.env}"
service_path="${2:-/etc/systemd/system/dune-target-safety-audit.service}"
timer_path="${3:-/etc/systemd/system/dune-target-safety-audit.timer}"
repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
service_template="$repo_root/config/systemd/dune-target-safety-audit.service"
timer_template="$repo_root/config/systemd/dune-target-safety-audit.timer"
...
if [[ ! -f "$service_template" || ! -f "$timer_template" ]]; then
  printf 'target safety audit systemd templates are missing\n' >&2
  exit 1
fi
...
sed \
  ...
  -e "s#^ExecStart=.*#ExecStart=$repo_root/scripts/target-safety-guard.sh $env_path#" \
