...
def chat_payload(message, channel_type, sender_funcom_id, sender_name, target_name):
...
def main():
    ...
    direct_variants = [
        ...
        ("DQ-private", "", target_queue, "Private", "text_chat"),
        ("DQ-whisper", "", target_queue, "Whisper", "text_chat"),
    ...
    try:
        ...
        for index, (label, exchange, routing_key, channel_type, prop_type) in enumerate(variants, start=1):
            message = f"{args.message_prefix} {label}"
            headers = None
            if exchange == "chat.intercept":
                headers = {"redirect_exchange": b"chat.whispers"}
            payload = chat_payload(message, channel_type, args.sender_funcom_id, args.sender_name, args.target_name)
            try:
