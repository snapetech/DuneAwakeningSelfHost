...
#define MAX_LUA_OBJECT_HANDLES 64
#define MAX_LUA_CLASS_METADATA 64
...
#define DEFAULT_MAX_MAPPING_BYTES (256ULL * 1024ULL * 1024ULL)
...
static const char *log_path(void) {
  const char *path = getenv("DUNE_CLIENT_PROBE_LOG");
  return path && path[0] ? path : "/tmp/dune-client-probe-loader.log";
}
...
static void append_log(const char *format, ...) {
  ...
  fprintf(file, "%s pid=%ld loader=client ", ts, (long)getpid());
...
static void safe_log_value(const char *value, char *out, size_t out_size) {
  ...
  size_t written = 0;
  for (const unsigned char *p = (const unsigned char *)value; p && *p && written + 1 < out_size; ++p) {
    unsigned char ch = *p;
...
static int is_target_process(const char *exe) {
  if (env_bool("DUNE_CLIENT_PROBE_FORCE", 0)) {
    return 1;
  }

  const char *targets = getenv("DUNE_CLIENT_PROBE_TARGET");
  ...
  return contains(base, "Dune") ||
         contains(base, "DuneSandbox") ||
         contains(base, "DuneAwakening");
}
...
static void log_modules(const char *phase) {
  if (!env_bool("DUNE_CLIENT_PROBE_LOG_MODULES", 1)) {
    return;
  }
...
static int path_matches_filters(const Mapping *mapping, const char *exe, char filters[][MAX_NEEDLE_LEN], size_t filter_count) {
  ...
  if (contains(mapping->path, "libdune_client_probe_loader.so")) {
    return 0;
  }
  ...
  if (filter_count == 0) {
    return contains(mapping->path, "/DuneSandbox/") ||
           contains(mapping->path, "/Dune Awakening/") ||
           contains(mapping->path, "/DuneAwakening/") ||
           contains(mapping->path, "/Engine/Binaries/Linux");
  }
...
static void append_preset(const char *preset, char needles[][MAX_NEEDLE_LEN], size_t *count) {
  if (strcmp(preset, "core") == 0) {
    ...
    for (size_t i = 0; i < sizeof(items) / sizeof(items[0]); ++i) add_needle(needles, count, items[i]);
  } else if (strcmp(preset, "client") == 0) {
    const char *items[] = {
      "LocalPlayer",
      "PlayerController",
      "GameViewport",
...
static void collect_needles(char needles[][MAX_NEEDLE_LEN], size_t *count) {
  const char *presets = getenv("DUNE_CLIENT_PROBE_SCAN_PRESETS");
  const char *extras = getenv("DUNE_CLIENT_PROBE_SCAN_STRINGS");
  const char *signatures = getenv("DUNE_CLIENT_PROBE_SCAN_SIGNATURES");
  NeedleCtx ctx = {needles, count};
  if ((!presets || !presets[0]) && (!extras || !extras[0]) && (!signatures || !signatures[0])) {
    presets = "core,ue,client,cheat,brt,deep-desert";
  }
...
static void collect_patterns(BytePattern *patterns, size_t *count) {
  ...
  parse_delimited(getenv("DUNE_CLIENT_PROBE_SCAN_SIGNATURES"), ';', pattern_callback, &ctx);
  collect_patterns_from_file(getenv("DUNE_CLIENT_PROBE_SCAN_SIGNATURES_FILE"), patterns, count);
}
...
static int mapping_is_runtime_discovery_target(const Mapping *mapping) {
  ...
  if (env_bool("DUNE_CLIENT_PROBE_UE_AUTO_DISCOVER_INCLUDE_ANONYMOUS_RW", 0) &&
      mapping_is_anonymous_writable_runtime_candidate(mapping)) {
    return 1;
  }
  if (env_bool("DUNE_CLIENT_PROBE_UE_AUTO_DISCOVER_INCLUDE_PRIVATE_RW", 0) &&
...
static uintptr_t target_image_base_for_ue_candidates(const Mapping *mappings, size_t count) {
  for (size_t i = 0; i < count; ++i) {
    if (mapping_is_target_image(&mappings[i]) &&
        (contains(mappings[i].path, "DuneSandbox") ||
         contains(mappings[i].path, "Dune/Binaries/Linux"))) {
...
static size_t collect_ue_anchors(UeAnchor *anchors) {
  ...
  parse_delimited(getenv("DUNE_CLIENT_PROBE_UE_ANCHORS"), ';', ue_anchor_callback, &ctx);
  if (env_bool("DUNE_CLIENT_PROBE_LOAD_ASSET_PACKAGE_SELF_TEST_ANCHOR", 0) && count < MAX_UE_ANCHORS) {
  ...
  if (env_bool("DUNE_CLIENT_PROBE_UE_SELF_TEST_ANCHOR", 0) && count < MAX_UE_ANCHORS) {
...
static int ue_anchor_signatures_configured(void) {
  const char *inline_patterns = getenv("DUNE_CLIENT_PROBE_UE_ANCHOR_SIGNATURES");
  const char *file_patterns = getenv("DUNE_CLIENT_PROBE_UE_ANCHOR_SIGNATURES_FILE");
...
static void collect_ue_anchor_signature_patterns(BytePattern *patterns, size_t *count) {
  PatternCtx ctx = {patterns, count};
  parse_delimited(getenv("DUNE_CLIENT_PROBE_UE_ANCHOR_SIGNATURES"), ';', pattern_callback, &ctx);
  collect_patterns_from_file(getenv("DUNE_CLIENT_PROBE_UE_ANCHOR_SIGNATURES_FILE"), patterns, count);
}
...
static size_t collect_ue_candidate_global_anchors(
  ...
  parse_delimited(getenv("DUNE_CLIENT_PROBE_UE_CANDIDATE_GLOBALS"), ';', ue_candidate_global_callback, &ctx);
  return count;
}
...
static size_t collect_ue_anchor_signature_anchors(
  ...
  parse_delimited(getenv("DUNE_CLIENT_PROBE_SCAN_PATH_FILTER"), ';', filter_callback, &filter_ctx);

  char exe[4096];
  executable_path(exe, sizeof(exe));
  size_t max_mapping_bytes = env_size("DUNE_CLIENT_PROBE_SCAN_MAX_MAPPING_BYTES", DEFAULT_MAX_MAPPING_BYTES);
  append_log(
...
static const Mapping *mapping_for_address(const Mapping *mappings, size_t count, uintptr_t address) {
  for (size_t i = 0; i < count; ++i) {
    if (mappings[i].start <= address && address < mappings[i].end) {
      return &mappings[i];
    }
  }
  return NULL;
...
static UeFNameOptions ue_fname_options(void) {
  ...
  options.pool_address = env_address_auto("DUNE_CLIENT_PROBE_UE_FNAME_POOL", 0);
  if (!options.pool_address) {
    options.pool_address = env_address_auto("DUNE_CLIENT_PROBE_UE_FNAME_POOL_ADDR", 0);
  }
  options.pool_offset = env_size_auto("DUNE_CLIENT_PROBE_UE_FNAME_POOL_OFFSET", 0);
  options.blocks_offset = env_size_auto("DUNE_CLIENT_PROBE_UE_FNAME_BLOCKS_OFFSET", DEFAULT_UE_FNAME_BLOCKS_OFFSET);
  options.stride = env_size_auto("DUNE_CLIENT_PROBE_UE_FNAME_STRIDE", DEFAULT_UE_FNAME_STRIDE);
  options.max_length = env_size("DUNE_CLIENT_PROBE_UE_FNAME_MAX_LENGTH", DEFAULT_UE_FNAME_MAX_LENGTH);
...
static UeObjectArrayOptions ue_object_array_options(void) {
  ...
  options.array_offset = env_size("DUNE_CLIENT_PROBE_UE_OBJECT_ARRAY_OFFSET", 0);
  options.item_size = env_size("DUNE_CLIENT_PROBE_UE_OBJECT_ARRAY_ITEM_SIZE", DEFAULT_UE_OBJECT_ITEM_SIZE);
  options.object_offset = env_size("DUNE_CLIENT_PROBE_UE_OBJECT_ARRAY_OBJECT_OFFSET", 0);
  options.chunk_size = env_size("DUNE_CLIENT_PROBE_UE_OBJECT_ARRAY_CHUNK_SIZE", DEFAULT_UE_OBJECT_CHUNK_SIZE);
  options.max_objects = env_size("DUNE_CLIENT_PROBE_UE_OBJECT_ARRAY_MAX_OBJECTS", DEFAULT_UE_OBJECT_ARRAY_MAX_OBJECTS);
...
static UeReflectionOptions ue_reflection_options(void) {
  ...
  options.next_offset = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_NEXT_OFFSET", DEFAULT_UE_REFLECTION_NEXT_OFFSET),
    DEFAULT_UE_REFLECTION_NEXT_OFFSET
  );
  options.super_offset = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_SUPER_OFFSET", DEFAULT_UE_REFLECTION_SUPER_OFFSET),
    DEFAULT_UE_REFLECTION_SUPER_OFFSET
  );
  options.children_offset = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_CHILDREN_OFFSET", DEFAULT_UE_REFLECTION_CHILDREN_OFFSET),
    DEFAULT_UE_REFLECTION_CHILDREN_OFFSET
  );
  options.child_properties_offset = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_CHILD_PROPERTIES_OFFSET", DEFAULT_UE_REFLECTION_CHILD_PROPERTIES_OFFSET),
    DEFAULT_UE_REFLECTION_CHILD_PROPERTIES_OFFSET
  );
  options.property_link_offset = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_PROPERTY_LINK_OFFSET", DEFAULT_UE_REFLECTION_PROPERTY_LINK_OFFSET),
    DEFAULT_UE_REFLECTION_PROPERTY_LINK_OFFSET
  );
  options.function_link_offset = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_FUNCTION_LINK_OFFSET", DEFAULT_UE_REFLECTION_FUNCTION_LINK_OFFSET),
    DEFAULT_UE_REFLECTION_FUNCTION_LINK_OFFSET
  );
  options.field_next_offset = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_FIELD_NEXT_OFFSET", DEFAULT_UE_REFLECTION_FIELD_NEXT_OFFSET),
    DEFAULT_UE_REFLECTION_FIELD_NEXT_OFFSET
  );
  options.property_array_dim_offset = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_PROPERTY_ARRAY_DIM_OFFSET", DEFAULT_UE_REFLECTION_PROPERTY_ARRAY_DIM_OFFSET),
    DEFAULT_UE_REFLECTION_PROPERTY_ARRAY_DIM_OFFSET
  );
  options.property_element_size_offset = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_PROPERTY_ELEMENT_SIZE_OFFSET", DEFAULT_UE_REFLECTION_PROPERTY_ELEMENT_SIZE_OFFSET),
    DEFAULT_UE_REFLECTION_PROPERTY_ELEMENT_SIZE_OFFSET
  );
  options.property_flags_offset = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_PROPERTY_FLAGS_OFFSET", DEFAULT_UE_REFLECTION_PROPERTY_FLAGS_OFFSET),
    DEFAULT_UE_REFLECTION_PROPERTY_FLAGS_OFFSET
  );
  options.property_offset_internal_offset = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_PROPERTY_OFFSET_INTERNAL_OFFSET", DEFAULT_UE_REFLECTION_PROPERTY_OFFSET_INTERNAL_OFFSET),
    DEFAULT_UE_REFLECTION_PROPERTY_OFFSET_INTERNAL_OFFSET
  );
  options.function_flags_offset = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_FUNCTION_FLAGS_OFFSET", DEFAULT_UE_REFLECTION_FUNCTION_FLAGS_OFFSET),
    DEFAULT_UE_REFLECTION_FUNCTION_FLAGS_OFFSET
  );
  options.container_child_scan_start = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_CONTAINER_CHILD_SCAN_START", DEFAULT_UE_REFLECTION_CONTAINER_CHILD_SCAN_START),
    DEFAULT_UE_REFLECTION_CONTAINER_CHILD_SCAN_START
  );
  options.container_child_scan_end = sane_reflection_offset(
    env_size_auto("DUNE_CLIENT_PROBE_UE_REFLECTION_CONTAINER_CHILD_SCAN_END", DEFAULT_UE_REFLECTION_CONTAINER_CHILD_SCAN_END),
  ...
  options.max_fields = env_size("DUNE_CLIENT_PROBE_UE_REFLECTION_MAX_FIELDS", DEFAULT_UE_REFLECTION_MAX_FIELDS);
  if (options.max_fields == 0 || options.max_fields > MAX_UE_REFLECTION_FIELDS) {
    options.max_fields = DEFAULT_UE_REFLECTION_MAX_FIELDS;
  }
  options.value_max_bytes = env_size("DUNE_CLIENT_PROBE_UE_REFLECTION_VALUE_MAX_BYTES", DEFAULT_UE_REFLECTION_VALUE_MAX_BYTES);
...
static size_t collect_ue_runtime_discovered_anchors(
  ...
  if (!env_bool("DUNE_CLIENT_PROBE_UE_AUTO_DISCOVER_ROOTS", 0)) {
    return anchor_count;
  }
  size_t max_mapping_bytes = env_size("DUNE_CLIENT_PROBE_UE_AUTO_DISCOVER_MAX_MAPPING_BYTES", DEFAULT_MAX_MAPPING_BYTES);
  size_t max_candidates = env_size("DUNE_CLIENT_PROBE_UE_AUTO_DISCOVER_MAX_CANDIDATES", 8);
  size_t min_object_array_elements = env_size("DUNE_CLIENT_PROBE_UE_AUTO_DISCOVER_MIN_OBJECT_ARRAY_ELEMENTS", 1);
  ...
  int include_anonymous_rw = env_bool("DUNE_CLIENT_PROBE_UE_AUTO_DISCOVER_INCLUDE_ANONYMOUS_RW", 0);
  int include_private_rw = env_bool("DUNE_CLIENT_PROBE_UE_AUTO_DISCOVER_INCLUDE_PRIVATE_RW", 0);
  size_t max_rejected_fname_samples = env_size("DUNE_CLIENT_PROBE_UE_AUTO_DISCOVER_MAX_REJECTED_FNAME_SAMPLES", 0);
...
static size_t collect_ue_hook_target_anchors(UeAnchor *anchors, const char *phase, const char *event_name) {
  ...
  const char *candidate_globals = getenv("DUNE_CLIENT_PROBE_UE_CANDIDATE_GLOBALS");
  int candidate_globals_configured = candidate_globals && *candidate_globals;
  int auto_discover_configured = env_bool("DUNE_CLIENT_PROBE_UE_AUTO_DISCOVER_ROOTS", 0);
...
static void log_ue_fname_resolver_reject(
  ...
  if (!env_bool("DUNE_CLIENT_PROBE_UE_FNAME_DIAGNOSTICS", 0)) {
    return;
  }
  UeFNameCandidateMetrics metrics;
  int ok = 0;
...
static void log_ue_fname_decode_diagnostic(
  ...
  int max_diagnostics = env_int("DUNE_CLIENT_PROBE_UE_FNAME_DIAGNOSTICS_MAX", 16);
...
static void run_mod_dispatch_self_test(const char *phase) {
  if (!env_bool("DUNE_CLIENT_PROBE_MOD_SELF_TEST", 0)) {
    return;
  }
...
static int load_lua_api_from_library(const char *library, LuaApi *api) {
  ...
  if (!api->new_state || !api->open_libs || !api->load_string || (!api->pcallk && !api->pcall) ||
      ...
      !api->type || !api->push_value || !api->ref || !api->rawgeti || !api->set_top ||
      !api->get_field ||
      !api->close) {
    dlclose(handle);
    memset(api, 0, sizeof(*api));
...
static int load_lua_api(LuaApi *api) {
  const char *configured = getenv("DUNE_CLIENT_PROBE_LUA_LIBRARY");
  if (configured && configured[0] && load_lua_api_from_library(configured, api)) {
    return 1;
  }
...
static int add_object_handle_to(
  ...
  snprintf(object->name, sizeof(object->name), "%.*s", (int)(sizeof(object->name) - 1), name && name[0] ? name : basename_ptr(path));
...
static int add_ue_candidate_object_handle(const char *path, const char *name, const char *class_name, uintptr_t address) {
  const char *candidate_name = name && name[0] ? name : basename_ptr(path);
  for (size_t i = 0; i < ue_candidate_object_handle_count; ++i) {
    const LuaObjectHandle *object = &ue_candidate_object_handles[i];
...
static void seed_runtime_probe_surfaces(void) {
  ...
  append_log(
    "event=lua-object-registry source=runtime-probe status=%s name=%s path=%s class=%s address=0x%lx registryCount=%lu registryProvenance=%s",
    registered_candidate ? "added" : "skipped",
    name,
    path,
    class_name,
...
static int push_lua_object_handle(LuaApi *api, LuaState *state, const LuaObjectHandle *object) {
  ...
  api->push_string(state, "probe-registry");
...
static int push_process_event_context_handle_or_nil(LuaApi *api, LuaState *state, uintptr_t address) {
  ...
  if (address == (uintptr_t)&lua_hook_registration_name[0] && lua_hook_registration_name[0]) {
    ...
    snprintf(synthetic.name, sizeof(synthetic.name), "%.*s", (int)(sizeof(synthetic.name) - 1), basename_ptr(lua_hook_registration_name));
    snprintf(synthetic.class_name, sizeof(synthetic.class_name), "%s", "UFunction");
    synthetic.address = address;
    if (push_lua_object_handle(api, state, &synthetic)) {
      return 2;
...
static int push_process_event_function_param_descriptors_or_nil(
  ...
  api->set_field(state, -2, "PathName");
  api->push_string(state, basename_ptr(lua_hook_registration_name));
  api->set_field(state, -2, "Name");
  api->push_string(state, "UFunction");
  api->set_field(state, -2, "ClassName");
...
static void reset_lua_hook_registration(void) {
  ...
  memset(lua_game_thread_queue, 0, sizeof(lua_game_thread_queue));
  memset(lua_scheduler_queue, 0, sizeof(lua_scheduler_queue));
  ...
  lua_reflection_probe_object_address = (uintptr_t)&ue_self_test_object;
...
static void lua_unref_hook_ref(LuaApi *api, LuaState *state, int *ref) {
  if (!api || !state || !ref || !lua_ref_is_valid(*ref) || !api->unref) {
    return;
  }
...
static int lua_store_hook_ref(LuaApi *api, LuaState *state, int stack_index, int *ref) {
  if (!api || !state || !ref || !api->type || !api->push_value || !api->ref) {
    return 0;
  }
  if (api->type(state, stack_index) != LUA_TFUNCTION_COMPAT) {
    return 0;
  }
...
static int lua_hook_registration_matches_process_event(
  ...
  const char *runtime_terminal = basename_ptr(ctx->function_path);
...
static int invoke_lua_hook_refs(LuaApi *api, LuaState *state) {
  ...
  for (int i = 0; i < lua_hook_registration_count && i < MAX_LUA_HOOK_REGISTRATIONS; ++i) {
    ...
    int status = invoke_lua_hook_ref(
      api,
      state,
      registration->pre_ref,
      &registration->pre_calls,
      &registration->pre_result,
      &registration->pre_is_number
    );
    if (status != 0) {
      final_status = status;
      continue;
  ...
  for (int i = 0; i < lua_hook_registration_count && i < MAX_LUA_HOOK_REGISTRATIONS; ++i) {
    ...
    int status = invoke_lua_hook_ref(
      api,
      state,
      registration->post_ref,
      &registration->post_calls,
      &registration->post_result,
      &registration->post_is_number
    );
    if (status != 0) {
      final_status = status;
      continue;
...
static int lua_register_hook_callback(LuaState *state) {
  ...
  size_t name_len = 0;
  const char *name = active_lua_api->to_lstring ? active_lua_api->to_lstring(state, 1, &name_len) : NULL;
  if (!name) {
    name = "";
    name_len = 0;
  }
  ...
  const char *terminal_name = basename_ptr(registration->name);
...
static int lua_find_first_of_callback(LuaState *state) {
  ...
  size_t class_len = 0;
  const char *class_name = active_lua_api->to_lstring ? active_lua_api->to_lstring(state, 1, &class_len) : NULL;
...
static int lua_find_objects_callback(LuaState *state) {
  ...
  const char *class_filter = active_lua_api->to_lstring ? active_lua_api->to_lstring(state, 1, NULL) : NULL;
  size_t count = 0;
...
static int lua_for_each_uobject_callback(LuaState *state) {
  ...
  if (!active_lua_api || !active_lua_api->type || !active_lua_api->push_value || !active_lua_api->set_top) {
    return 0;
  }
  if (active_lua_api->type(state, 1) != LUA_TFUNCTION_COMPAT) {
    return 0;
  }
  const char *class_filter = active_lua_api->to_lstring ? active_lua_api->to_lstring(state, 2, NULL) : NULL;
  for (size_t i = 0; i < lua_object_handle_count; ++i) {
    const LuaObjectHandle *object = &lua_object_handles[i];
    if (!lua_object_class_matches(object, class_filter)) {
      continue;
...
static int lua_is_a_callback(LuaState *state) {
  ...
  const char *class_name = active_lua_api->to_lstring ? active_lua_api->to_lstring(state, 2, NULL) : NULL;
  const LuaObjectHandle *object = find_lua_object_by_address(address);
...
static int lua_load_asset_callback(LuaState *state) {
  ...
  const char *query = active_lua_api->to_lstring ? active_lua_api->to_lstring(state, 1, NULL) : NULL;
  const LuaObjectHandle *object = find_lua_object_by_path_or_name(query);
  if (object && push_lua_object_handle(active_lua_api, state, object)) {
    ++lua_load_asset_hits;
    return 1;
  }
  int package_requested = env_bool("DUNE_CLIENT_PROBE_LOAD_ASSET_PACKAGE_DRY_RUN", 0);
  ...
  if (package_requested) {
    ...
    if (env_bool("DUNE_CLIENT_PROBE_LOAD_ASSET_PACKAGE_DRY_RUN", 0)) {
    ...
    const int invoke_enabled = env_bool("DUNE_CLIENT_PROBE_ALLOW_LOAD_ASSET_PACKAGE_INVOKE", 0);
    char safe_query[256];
    ...
    append_log(
      ...
      env_bool("DUNE_CLIENT_PROBE_LOAD_ASSET_PACKAGE_DRY_RUN", 0) ? "true" : "false"
    );
  }
  active_lua_api->push_nil(state);
  return 1;
...
static int lua_get_load_asset_package_tchar_verification_state_callback(LuaState *state) {
  ...
  const char *unit_raw = getenv("DUNE_CLIENT_PROBE_TCHAR_UNIT_BYTES");
  const char *evidence = getenv("DUNE_CLIENT_PROBE_TCHAR_EVIDENCE");
  ...
  int verification_enabled = env_bool("DUNE_CLIENT_PROBE_CONFIRM_TCHAR_LAYOUT", 0);
  uintptr_t target = lua_load_asset_backend_static_load_object_address;
  const char *target_name = "StaticLoadObject";
  if (!target && lua_load_asset_backend_load_object_address) {
    target = lua_load_asset_backend_load_object_address;
  ...
  int layout_verified = verification_enabled && unit_match && target != 0;
  ...
  active_lua_api->push_string(state, "DUNE_CLIENT_PROBE_TCHAR_UNIT_BYTES");
  active_lua_api->set_field(state, -2, "UnitBytesEnv");
  active_lua_api->push_string(state, "DUNE_CLIENT_PROBE_TCHAR_EVIDENCE");
  active_lua_api->set_field(state, -2, "EvidenceEnv");
  active_lua_api->push_string(state, "DUNE_CLIENT_PROBE_CONFIRM_TCHAR_LAYOUT");
...
static int lua_get_load_asset_package_call_frame_verification_state_callback(LuaState *state) {
  ...
  const char *abi_evidence = getenv("DUNE_CLIENT_PROBE_LOAD_ASSET_PACKAGE_ABI_EVIDENCE");
  int abi_evidence_provided = abi_evidence && abi_evidence[0] != '\0';
  int abi_verification_enabled = env_bool("DUNE_CLIENT_PROBE_CONFIRM_LOAD_ASSET_PACKAGE_ABI", 0);
  int abi_verified = abi_evidence_provided && abi_verification_enabled && target != 0 && lua_load_asset_backend_package_target_image;

  size_t candidate_unit_bytes = sizeof(wchar_t);
  unsigned long long observed_unit_bytes = 0;
  const char *tchar_unit_raw = getenv("DUNE_CLIENT_PROBE_TCHAR_UNIT_BYTES");
  const char *tchar_evidence = getenv("DUNE_CLIENT_PROBE_TCHAR_EVIDENCE");
  ...
  int tchar_verification_enabled = env_bool("DUNE_CLIENT_PROBE_CONFIRM_TCHAR_LAYOUT", 0);
  ...
  active_lua_api->push_integer(state, argument_count);
  active_lua_api->set_field(state, -2, "ArgumentCount");
  active_lua_api->push_string(state, "DUNE_CLIENT_PROBE_LOAD_ASSET_PACKAGE_ABI_EVIDENCE");
  active_lua_api->set_field(state, -2, "AbiEvidenceEnv");
  active_lua_api->push_string(state, "DUNE_CLIENT_PROBE_CONFIRM_LOAD_ASSET_PACKAGE_ABI");
...
static int load_asset_package_crash_guard_armed(void) {
  return load_asset_package_crash_guard_available() &&
         env_bool("DUNE_CLIENT_PROBE_ENABLE_LOAD_ASSET_PACKAGE_CRASH_GUARD", 0);
}
...
static int lua_get_load_asset_package_crash_guard_state_callback(LuaState *state) {
  ...
  int enabled = env_bool("DUNE_CLIENT_PROBE_ENABLE_LOAD_ASSET_PACKAGE_CRASH_GUARD", 0);
  ...
  active_lua_api->push_string(state, "DUNE_CLIENT_PROBE_ENABLE_LOAD_ASSET_PACKAGE_CRASH_GUARD");
...
static int lua_get_load_asset_package_native_call_adapter_state_callback(LuaState *state) {
  ...
  int tchar_layout_verified = tchar_verification_enabled && tchar_unit_match && target != 0 && lua_load_asset_backend_package_target_image;
  int call_frame_ready = bounded_input && target != 0 && lua_load_asset_backend_package_target_image && abi_verified && tchar_layout_verified;
  int invoke_enabled = env_bool("DUNE_CLIENT_PROBE_ALLOW_LOAD_ASSET_PACKAGE_INVOKE", 0);
  ...
  int final_invoke_confirmed = env_bool("DUNE_CLIENT_PROBE_CONFIRM_LOAD_ASSET_PACKAGE_NATIVE_CALL", 0);
  ...
  active_lua_api->push_string(state, "DUNE_CLIENT_PROBE_CONFIRM_LOAD_ASSET_PACKAGE_NATIVE_CALL");
...
static int lua_invoke_load_asset_package_native_callback(LuaState *state) {
  ...
  const int abi_verification_enabled = env_bool("DUNE_CLIENT_PROBE_CONFIRM_LOAD_ASSET_PACKAGE_ABI", 0);
  ...
  const int tchar_verification_enabled = env_bool("DUNE_CLIENT_PROBE_CONFIRM_TCHAR_LAYOUT", 0);
  ...
  const int final_invoke_confirmed = env_bool("DUNE_CLIENT_PROBE_CONFIRM_LOAD_ASSET_PACKAGE_NATIVE_CALL", 0);
...
static void lua_copy_string_arg_or_field(
  ...
  if (api && state && field && api->type && api->type(state, index) == LUA_TTABLE_COMPAT && api->get_field && api->set_top) {
    ...
    value = api->to_lstring ? api->to_lstring(state, -1, &value_len) : NULL;
    if (value && value[0]) {
...
static int lua_ftext_callback(LuaState *state) {
  ...
  const char *text = active_lua_api->to_lstring ? active_lua_api->to_lstring(state, 1, NULL) : NULL;
...
static int lua_unreal_version_major(void) {
  return env_int("DUNE_CLIENT_PROBE_UNREAL_VERSION_MAJOR", 5);
}

static int lua_unreal_version_minor(void) {
  return env_int("DUNE_CLIENT_PROBE_UNREAL_VERSION_MINOR", 0);
}
...
static int lua_iterate_game_directories_callback(LuaState *state) {
  ...
  const char *configured = getenv("DUNE_CLIENT_PROBE_GAME_DIR");
...
static uintptr_t lua_object_address_arg(LuaApi *api, LuaState *state, int index) {
  ...
  if (api->type && api->type(state, index) == LUA_TTABLE_COMPAT && api->get_field && api->set_top) {
  ...
  size_t query_len = 0;
  const char *query = api->to_lstring ? api->to_lstring(state, index, &query_len) : NULL;
  if (query && query[0]) {
    const LuaObjectHandle *object = find_lua_object_by_path_or_name(query);
...
static int push_lua_property_descriptor(
  ...
  if (contains_ci(safe_class, "EnumProperty")) {
    ...
    api->push_string(state, "/Script/DuneProbe.EProbeMode");
...
static int push_self_test_property_descriptor(
  ...
  if (owner_address != (uintptr_t)&ue_self_test_object || !property || !property[0]) {
    return 0;
  }
...
static int lua_property_get_value_callback(LuaState *state) {
  ...
  if (owner != (uintptr_t)&ue_self_test_object) {
...
static int lua_property_set_value_callback(LuaState *state) {
  ...
  if (owner == (uintptr_t)&ue_self_test_object && strcmp(name, "ProbeValue") == 0 && is_number) {
    lua_reflection_probe_value = value;
    ++lua_reflection_descriptor_value_set_hits;
    active_lua_api->push_boolean(state, 1);
    return 1;
  }
  if (owner == (uintptr_t)&ue_self_test_object && strcmp(name, "ProbeBool") == 0) {
    int is_boolean = 0;
  ...
  if (owner == (uintptr_t)&ue_self_test_object && strcmp(name, "ProbeFloat") == 0 && is_real_number) {
    lua_reflection_probe_float_value = (float)number_value;
    ++lua_reflection_descriptor_value_set_hits;
    active_lua_api->push_boolean(state, 1);
    return 1;
  }
  if (owner == (uintptr_t)&ue_self_test_object && strcmp(name, "ProbeDouble") == 0 && is_real_number) {
    lua_reflection_probe_double_value = number_value;
    ++lua_reflection_descriptor_value_set_hits;
    active_lua_api->push_boolean(state, 1);
    return 1;
  }
  if (owner == (uintptr_t)&ue_self_test_object && strcmp(name, "ProbeName") == 0) {
  ...
  if (owner == (uintptr_t)&ue_self_test_object && strcmp(name, "ProbeString") == 0) {
    size_t value_len = 0;
    const char *string_value = active_lua_api->to_lstring ? active_lua_api->to_lstring(state, 2, &value_len) : NULL;
    if (string_value) {
  ...
  if (owner == (uintptr_t)&ue_self_test_object && strcmp(name, "ProbeText") == 0) {
  ...
  if (owner == (uintptr_t)&ue_self_test_object && strcmp(name, "ProbeEnum") == 0 && is_number &&
      value >= 0 && value <= 255) {
  ...
  if (owner == (uintptr_t)&ue_self_test_object && strcmp(name, "ProbeObject") == 0) {
  ...
  if (env_bool("DUNE_CLIENT_PROBE_LUA_REFLECTION_RAW_SET_ENABLED", 0)) {
    int matched_field_name = 0;
    const UeReflectionPropertyCandidate *candidate = find_ue_reflection_property_candidate(owner, name, &matched_field_name);
...
static int lua_property_import_text_callback(LuaState *state) {
  ...
  int ok = 0;
  if (owner == (uintptr_t)&ue_self_test_object && text) {
...
static int lua_enum_property_get_enum_callback(LuaState *state) {
  ...
  if (contains_ci(class_name, "EnumProperty")) {
    LuaObjectHandle synthetic;
    memset(&synthetic, 0, sizeof(synthetic));
    snprintf(synthetic.path, sizeof(synthetic.path), "%s", enum_path[0] ? enum_path : "/Script/DuneProbe.EProbeMode");
...
static int lua_set_property_get_element_property_callback(LuaState *state) {
  ...
  if (contains_ci(class_name, "SetProperty")) {
    ...
    return push_lua_property_descriptor(
      active_lua_api,
      state,
      "/RuntimeProbe/Property/SetElement",
      0,
      element_name,
      element_class_name,
      element_type,
...
static int lua_map_property_get_value_property_callback(LuaState *state) {
  ...
  if (contains_ci(class_name, "MapProperty")) {
    ...
    return push_lua_property_descriptor(
      active_lua_api,
      state,
      "/RuntimeProbe/Property/MapValue",
      0,
      value_name,
      value_class_name,
      value_type,
...
static void ue_function_descriptor_owner_name(
  ...
  const char *terminal = basename_ptr(path);
...
static int lua_struct_for_each_function_callback(LuaState *state) {
  ...
  int self_test_iterate =
    address == (uintptr_t)&ue_self_test_object ||
    strcmp(class_name, "DuneProbeSelfTestClass") == 0 ||
    strcmp(name, "DuneProbeSelfTestClass") == 0;
  int owner_iterate = 0;
...
static int lua_struct_for_each_property_callback(LuaState *state) {
  ...
  if (address == (uintptr_t)&ue_self_test_object) {
...
static int lua_get_property_value_callback(LuaState *state) {
  ...
  uintptr_t address = lua_object_address_arg(active_lua_api, state, 1);
  const char *property = active_lua_api->to_lstring ? active_lua_api->to_lstring(state, 2, NULL) : NULL;
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeValue") == 0) {
    ++lua_reflection_get_property_hits;
    active_lua_api->push_integer(state, lua_reflection_probe_value);
    return 1;
  }
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeBool") == 0) {
    ++lua_reflection_get_property_hits;
    active_lua_api->push_boolean(state, lua_reflection_probe_bool_value);
    return 1;
  }
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeFloat") == 0) {
    ++lua_reflection_get_property_hits;
    active_lua_api->push_number(state, (double)lua_reflection_probe_float_value);
    return 1;
  }
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeDouble") == 0) {
    ++lua_reflection_get_property_hits;
    active_lua_api->push_number(state, lua_reflection_probe_double_value);
    return 1;
  }
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeName") == 0) {
    if (push_lua_fname_table(
          active_lua_api,
          state,
          lua_reflection_probe_name_string,
  ...
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeString") == 0) {
    ++lua_reflection_get_property_hits;
    active_lua_api->push_string(state, lua_reflection_probe_string_value);
    return 1;
  }
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeText") == 0) {
    if (push_lua_ftext_table(active_lua_api, state, lua_reflection_probe_text_value)) {
      ++lua_reflection_get_property_hits;
      return 1;
    }
  }
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeEnum") == 0) {
    ++lua_reflection_get_property_hits;
    active_lua_api->push_integer(state, (long long)lua_reflection_probe_enum_value);
    return 1;
  }
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeObject") == 0) {
    const LuaObjectHandle *object = find_lua_object_by_path_or_name("/Script/DuneProbe.SelfTestObject");
    if (object && object->address == lua_reflection_probe_object_address &&
        push_lua_object_handle(active_lua_api, state, object)) {
      ++lua_reflection_get_property_hits;
  ...
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ClassName") == 0) {
    ++lua_reflection_get_property_hits;
...
static int lua_set_property_value_callback(LuaState *state) {
  ...
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeValue") == 0 && is_number) {
    lua_reflection_probe_value = value;
  ...
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeFloat") == 0 && is_real_number) {
    lua_reflection_probe_float_value = (float)number_value;
    ++lua_reflection_set_property_hits;
    active_lua_api->push_boolean(state, 1);
    return 1;
  }
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeDouble") == 0 && is_real_number) {
    lua_reflection_probe_double_value = number_value;
    ++lua_reflection_set_property_hits;
    active_lua_api->push_boolean(state, 1);
    return 1;
  ...
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeString") == 0) {
    size_t value_len = 0;
    const char *string_value = active_lua_api->to_lstring ? active_lua_api->to_lstring(state, 3, &value_len) : NULL;
    if (string_value) {
      size_t copy_len = value_len;
      if (copy_len + 1 > sizeof(lua_reflection_probe_string_value)) {
        copy_len = sizeof(lua_reflection_probe_string_value) - 1;
  ...
  if (address == (uintptr_t)&ue_self_test_object && property && strcmp(property, "ProbeEnum") == 0 && is_number &&
      value >= 0 && value <= 255) {
    lua_reflection_probe_enum_value = (uint8_t)value;
    ++lua_reflection_set_property_hits;
    active_lua_api->push_boolean(state, 1);
  ...
  if (env_bool("DUNE_CLIENT_PROBE_LUA_REFLECTION_RAW_SET_ENABLED", 0) && is_number &&
...
static int lua_call_function_callback(LuaState *state) {
  ...
  if (address == (uintptr_t)&ue_self_test_object && function && strcmp(function, "DoubleProbeValue") == 0) {
    ++lua_reflection_call_function_hits;
  ...
  if (address == (uintptr_t)&ue_self_test_object && function && strcmp(function, "IsProbeBoolSet") == 0) {
    ++lua_reflection_call_function_hits;
    active_lua_api->push_boolean(state, lua_reflection_probe_bool_value);
    if (invoke_lua_call_function_hooks(state, 0, function, args, 1, 1)) {
      return 1;
...
static int lua_invoke_process_event_native_callback(LuaState *state) {
  ...
  const int native_non_self_test_enabled = env_bool("DUNE_CLIENT_PROBE_ALLOW_NON_SELF_TEST_PROCESS_EVENT_INVOKE", 0);
...
static void run_lua_process_event_native_invoke_self_test(const char *phase) {
  ...
  const char *script =
    "local o=StaticFindObject('/Script/DuneProbe.SelfTestObject');"
...
static int add_lua_mod_script(LuaModScriptList *list, const char *path, const char *name) {
  ...
  snprintf(script->name, sizeof(script->name), "%s", (name && name[0]) ? name : basename_ptr(path));
  return 1;
}
...
static void lua_mod_script_callback(char *item, void *ctx) {
  LuaModScriptListCtx *list_ctx = (LuaModScriptListCtx *)ctx;
  add_lua_mod_script(list_ctx->list, item, basename_ptr(item));
}

static void collect_lua_mod_scripts_from_env(LuaModScriptList *list) {
  LuaModScriptListCtx ctx;
  ctx.list = list;
  parse_delimited(getenv("DUNE_CLIENT_PROBE_LUA_MOD_SCRIPTS"), ';', lua_mod_script_callback, &ctx);
}

static void collect_lua_mod_scripts_from_root(LuaModScriptList *list) {
  const char *root = getenv("DUNE_CLIENT_PROBE_LUA_MOD_ROOT");
  if (!root || !root[0]) {
    return;
  }
...
static char *read_lua_script_file(const char *path, size_t *length, const char **reason) {
  if (length) {
    *length = 0;
  }
  if (reason) {
  ...
  char *buffer = (char *)malloc(MAX_LUA_SCRIPT_BYTES + 1);
  if (!buffer) {
    fclose(file);
    if (reason) {
  ...
  size_t got = fread(buffer, 1, MAX_LUA_SCRIPT_BYTES + 1, file);
...
static int run_lua_mod_script(LuaApi *api, LuaState *state, const char *phase, const LuaModScript *script) {
  char safe_path[MAX_LUA_SCRIPT_PATH];
  char safe_name[MAX_LUA_SCRIPT_NAME];
  safe_log_value(script->path, safe_path, sizeof(safe_path));
  safe_log_value(script->name, safe_name, sizeof(safe_name));
  const char *reason = "unknown";
  size_t script_len = 0;
  char *source = read_lua_script_file(script->path, &script_len, &reason);
  if (!source) {
    char safe_reason[64];
    safe_log_value(reason, safe_reason, sizeof(safe_reason));
    append_log("event=lua-mod-script phase=%s status=read-failed name=%s path=%s reason=%s", phase, safe_name, safe_path, safe_reason);
    return 0;
  }
  ...
  register_lua_compat_metadata_tables(api, state);
  char *wrapped_source = build_lua_mod_script_source(source, script_len, lua_current_mod_path);
  int load_status = wrapped_source ? api->load_string(state, wrapped_source) : 1;
  ...
  append_log(
    "event=lua-mod-script phase=%s status=%s name=%s path=%s loadStatus=%d callStatus=%d error=%s hooksBefore=%d hooksAfter=%d scriptBytes=%lu",
    phase,
    status,
    safe_name,
    safe_path,
  ...
  return load_status == 0 && call_status == 0;
}

static size_t load_lua_live_mods_into_state(LuaApi *api, LuaState *state, const char *phase) {
  if (!api || !state || !env_bool("DUNE_CLIENT_PROBE_LUA_MODS_ENABLED", 0)) {
    return 0;
  }
  LuaModScriptList list;
  ...
  append_log(
    ...
    (unsigned long)list.skipped,
    (unsigned long)list.manifest_entries,
    (unsigned long)list.manifest_disabled
  );
  size_t loaded = 0;
  size_t failed = 0;
  ...
  append_log(
    ...
    phase,
    list.count > 0 && failed == 0 ? "passed" : (list.count == 0 ? "no-scripts" : "failed"),
    (unsigned long)list.count,
...
static void run_lua_mod_entrypoints(const char *phase) {
  if (!env_bool("DUNE_CLIENT_PROBE_LUA_MODS_ENABLED", 0)) {
    return;
  }
  ...
  memset(&api, 0, sizeof(api));
  if (!load_lua_api(&api)) {
    append_log("event=lua-mod-finish phase=%s status=library-missing", phase);
    return;
  }
  ...
  if (!state) {
    append_log("event=lua-mod-finish phase=%s status=state-failed library=%s", phase, safe_library);
    dlclose(api.handle);
    return;
  }
  api.open_libs(state);
  ...
  collect_lua_mod_scripts_from_root(&list);
  append_log(
    "event=lua-mod-start phase=%s status=running library=%s scripts=%lu skipped=%lu manifestEntries=%lu manifestDisabled=%lu",
    phase,
    safe_library,
    (unsigned long)list.count,
    (unsigned long)list.skipped,
  ...
  if (env_bool("DUNE_CLIENT_PROBE_LUA_MOD_DISPATCH_SELF_TEST", 0) && lua_hook_registration_count > 0) {
    callback_status = invoke_lua_hook_refs(&api, state);
    char safe_hook[256];
...
static void run_lua_dispatch_self_test(const char *phase) {
  if (!env_bool("DUNE_CLIENT_PROBE_LUA_SELF_TEST", 0)) {
    return;
  }
  reset_lua_hook_registration();
  LuaApi api;
  ...
  const char *script = getenv("DUNE_CLIENT_PROBE_LUA_SELF_TEST_SCRIPT");
  if (!script || !script[0]) {
    script = "local o=StaticFindObject('/Script/DuneProbe.SelfTestObject'); local fo=FindObject('SelfTestObject'); local f=FindFirstOf('DuneProbeSelfTestClass'); local known=GetKnownObjects(); local matches=FindObjects('DuneProbeSelfTestClass'); local allOf=FindAllOf('DuneProbeSelfTestClass'); local asset=LoadAsset('/Script/DuneProbe.SelfTestObject'); local assetBackend=GetLoadAssetBackendState(); local seen=0; ForEachUObject(function(x) if x and IsA(x,'DuneProbeSelfTestClass') then seen=seen+1 end end); local cls=o and o:GetClass(); local cdo=cls and cls:GetDefaultObject(); local cdo2=cls and cls:GetDefaultObj(); local sup=cls and cls:GetSuper(); local sup2=cls and cls:GetSuperClass(); local methodOk=o and cls and cdo and cdo2 and sup and sup2 and cdo.Name=='Default__'..cls.Name and cdo2.Name==cdo.Name and sup.Name=='UObject' and sup2.Name==sup.Name and o:GetName()=='SelfTestObject' and o:GetPathName()==o.PathName and o:GetAddress()==o.Address and o:IsValid() and o:GetFullName()=='DuneProbeSelfTestClass /Script/DuneProbe.SelfTestObject' and o:type()=='UObject' and o:GetFName()=='SelfTestObject' and o:GetOuter()==nil and o:GetWorld()==nil and cls.Name=='DuneProbeSelfTestClass' and cls.ClassName=='UClass' and cls:type()=='UClass' and not cls:IsValid() and o:HasAllFlags(0) and not o:HasAnyFlags(1) and not o:HasAnyInternalFlags(1); local async=0; local asyncId=ExecuteAsync(function() async=async+1 end); local delayId=ExecuteWithDelay(1,function() async=async+1 end); local loopId=LoopAsync(1,function() async=async+1 return false end); local cancelledAsync=0; local cancelId=ExecuteWithDelay(1,function() cancelledAsync=cancelledAsync+1 end); local schedulerCancelOk=cancelId and CancelScheduledCallback(cancelId); local schedulerDrained=DrainSchedulerQueue(); local fn=FName('ProbeName'); local ft=FText('ProbeText'); local kb=RegisterKeyBind('P',function(ctx,key) return ctx==o and key=='P' end); local cancelledKeyHits=0; local kb2=RegisterKeyBind('Q',function() cancelledKeyHits=cancelledKeyHits+1 return true end); local keyCancelOk=kb2 and UnregisterKeyBind(kb2) and not IsKeyBindRegistered('Q') and not DuneProbeDispatchKeyBind(o,'Q') and cancelledKeyHits==0; local cancelledConsoleHits=0; local cc=RegisterConsoleCommandHandler('cancelprobe',function() cancelledConsoleHits=cancelledConsoleHits+1 return true end); local consoleCancelOk=cc and UnregisterConsoleCommandHandler(cc) and not DuneProbeDispatchConsoleCommand(o,'cancelprobe') and cancelledConsoleHits==0; local compatOk=async==3 and asyncId and delayId and loopId and schedulerCancelOk and schedulerDrained==3 and cancelledAsync==0 and fn.Name=='ProbeName' and fn:ToString()=='ProbeName' and fn:GetComparisonIndex()==0 and ft.Text=='ProbeText' and kb and IsKeyBindRegistered('P') and DuneProbeDispatchKeyBind(o,'P') and keyCancelOk and consoleCancelOk and RegisterConsoleCommandHandler('probe',function(ctx,cmd,args) return ctx==o and cmd=='probe' and args=='arg' and false end) and RegisterConsoleCommandGlobalHandler(function(ctx,cmd,args) return ctx==o and cmd=='probe' and args=='arg' end) and DuneProbeDispatchConsoleCommand(o,'probe arg') and RegisterCustomEvent('probe',function() end) and RegisterLoadMapPreHook(function() end) and RegisterLoadMapPostHook(function() end) and RegisterBeginPlayPreHook(function() end) and RegisterBeginPlayPostHook(function() end) and RegisterInitGameStatePreHook(function() end) and RegisterInitGameStatePostHook(function() end) and RegisterProcessConsoleExecPreHook(function() end) and RegisterProcessConsoleExecPostHook(function() end) and RegisterCallFunctionByNameWithArgumentsPreHook(function() end) and RegisterCallFunctionByNameWithArgumentsPostHook(function() end); local up,uq=RegisterHook('/Script/DuneProbe.Temp:Function',function()return -1 end,function()return -1 end); local unregOk=up and uq and uq==up+1; UnregisterHook('/Script/DuneProbe.Temp:Function',up,uq); NotifyOnNewObject('DuneProbeSelfTestClass', function(x) if x and x.Name=='ConstructedProbe' then return 17 end return -17 end); local c=StaticConstructObject(f,o,'ConstructedProbe'); local gameThreadId=ExecuteInGameThread(function() return 9 end); local drained=DrainGameThreadQueue(); local ok=o and fo and f and known and matches and allOf and asset and c and methodOk and compatOk and unregOk and gameThreadId and gameThreadId>0 and drained==1 and o.PathName=='/Script/DuneProbe.SelfTestObject' and fo.Address==o.Address and asset.Address==o.Address and assetBackend and assetBackend.Backend=='registry' and assetBackend.RegistryFallback and not assetBackend.PackageBackendArmed and assetBackend.ContractVersion==1 and f.ClassName=='DuneProbeSelfTestClass' and known[o.PathName] and known.Count>=1 and matches.Count>=1 and allOf.Count>=1 and seen>=1 and IsA(o,'DuneProbeSelfTestClass') and IsA(o,'UObject') and c.Name=='ConstructedProbe' and c.ClassName=='DuneProbeSelfTestClass'; return (ok and 40 or -100) + RegisterHook('/Script/DuneProbe.SelfTest:Function', function() return 11 end, function() return 31 end)";
  }
  char safe_library[256];
  safe_log_value(api.library, safe_library, sizeof(safe_library));
  LuaState *state = api.new_state();
...
static void run_lua_reflection_self_test(const char *phase) {
  if (!env_bool("DUNE_CLIENT_PROBE_LUA_REFLECTION_SELF_TEST", 0)) {
    return;
  }
  reset_lua_hook_registration();
  LuaApi api;
  ...
  const char *script = getenv("DUNE_CLIENT_PROBE_LUA_REFLECTION_SELF_TEST_SCRIPT");
  int raw_set_self_test = env_bool("DUNE_CLIENT_PROBE_LUA_REFLECTION_RAW_SET_ENABLED", 0);
  if (!script || !script[0]) {
  ...
  append_log(
    ...
    lua_reflection_get_property_hits,
    lua_reflection_raw_property_hits,
    lua_reflection_raw_property_value,
    lua_reflection_named_property_hits,
    lua_reflection_raw_property_set_hits,
    lua_reflection_raw_property_set_value,
    lua_reflection_array_inner_property_hits,
    lua_reflection_enum_property_hits,
    lua_reflection_enum_underlying_property_hits,
    lua_reflection_set_element_property_hits,
    ...
    lua_reflection_call_function_calls,
    lua_reflection_call_function_hits,
    lua_reflection_probe_value,
    lua_reflection_probe_bool_value ? "true" : "false",
    (double)lua_reflection_probe_float_value,
    lua_reflection_probe_double_value,
    lua_reflection_probe_name_string,
    lua_reflection_probe_string_value,
    lua_reflection_probe_text_value,
    (unsigned long)lua_reflection_probe_object_address,
    (unsigned long)lua_object_handle_count,
    (unsigned long)ue_candidate_object_handle_count,
    (unsigned long)strlen(script)
  );
...
static void run_hook_dispatch_self_test(const char *phase) {
  if (!env_bool("DUNE_CLIENT_PROBE_HOOK_SELF_TEST", 0)) {
    return;
  }
  reset_hook_dispatch_slots();
  ...
  HookSelfTestFn entry = hook_self_test_entry;
  int before = entry(41);
...
static ProcessEventSelfTestFn volatile process_event_self_test_entry = process_event_self_test_target;
...
static void build_process_event_live_lua_script(char *script, size_t script_size) {
  ...
  append_lua_reflection_script_chunk(
    script,
    script_size,
    "RegisterHook('/Script/DuneProbe.NotTarget:Function', function() return -99 end, function() return -99 end); "
...
static int prepare_process_event_live_lua_dispatch(const char *phase) {
  process_event_live_lua_enabled = env_bool("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_LIVE_LUA_DISPATCH", 0);
  ...
  const char *script = getenv("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_LIVE_LUA_SCRIPT");
  if (!script || !script[0]) {
...
static const char *process_event_hook_target_source(uintptr_t target) {
  if (target && env_address_auto("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_HOOK_ADDRESS", 0) == target) {
    return "explicit-hook-address";
  }
  if (target && env_address_auto("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_ADDRESS", 0) == target) {
...
static const char *process_event_live_hook_target_source(uintptr_t target) {
  if (target && env_address_auto("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_LIVE_HOOK_ADDRESS", 0) == target) {
...
static uintptr_t configured_process_event_hook_target(const char *phase, int *self_test_target) {
  ...
  if (env_bool("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_HOOK_SELF_TEST_TARGET", 0)) {
    if (self_test_target) {
      *self_test_target = 1;
    }
  ...
  uintptr_t target = env_address_auto("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_HOOK_ADDRESS", 0);
  if (!target) {
    target = env_address_auto("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_ADDRESS", 0);
  }
  if (target) {
    return target;
  }
...
static void run_process_event_hook_probe(const char *phase) {
  if (!env_bool("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_HOOK_PROBE", 0)) {
    return;
  }

  int self_test_target = 0;
  uintptr_t target = configured_process_event_hook_target(phase, &self_test_target);
  int install = env_bool("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_HOOK_INSTALL", 0);
  int call_self_test = env_bool("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_HOOK_CALL_SELF_TEST", self_test_target);
  if (!target) {
...
static const char *call_function_hook_target_source(uintptr_t target) {
  if (target && env_address_auto("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_HOOK_ADDRESS", 0) == target) {
    return "explicit-hook-address";
  }
  if (target && env_address_auto("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_ADDRESS", 0) == target) {
...
static const char *call_function_live_hook_target_source(uintptr_t target) {
  if (target && env_address_auto("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_LIVE_HOOK_ADDRESS", 0) == target) {
    return "explicit-live-hook-address";
  }
...
static uintptr_t configured_call_function_hook_target(const char *phase, int *self_test_target) {
  ...
  if (env_bool("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_HOOK_SELF_TEST_TARGET", 0)) {
    if (self_test_target) {
      *self_test_target = 1;
    }
  ...
  uintptr_t target = env_address_auto("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_HOOK_ADDRESS", 0);
  if (!target) {
    target = env_address_auto("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_ADDRESS", 0);
  }
  if (target) {
    return target;
  }
...
static void run_call_function_hook_probe(const char *phase) {
  if (!env_bool("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_HOOK_PROBE", 0)) {
    return;
  }

  int self_test_target = 0;
  uintptr_t target = configured_call_function_hook_target(phase, &self_test_target);
  int install = env_bool("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_HOOK_INSTALL", 0);
  int call_self_test = env_bool("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_HOOK_CALL_SELF_TEST", self_test_target);
  if (!target) {
...
static uintptr_t configured_call_function_live_hook_target(const char *phase, int *self_test_target) {
  ...
  if (env_bool("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_LIVE_HOOK_SELF_TEST_TARGET", 0)) {
    if (self_test_target) {
      *self_test_target = 1;
    }
  ...
  uintptr_t target = env_address_auto("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_LIVE_HOOK_ADDRESS", 0);
  if (!target) {
    target = env_address_auto("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_HOOK_ADDRESS", 0);
  }
  if (!target) {
    target = env_address_auto("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_ADDRESS", 0);
  }
...
static void install_call_function_live_hook(const char *phase) {
  if (!env_bool("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_LIVE_HOOK", 0)) {
    return;
  }
  ...
  int call_self_test = env_bool("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_LIVE_HOOK_CALL_SELF_TEST", self_test_target);
  call_function_live_hook_log_calls = env_bool("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_LIVE_HOOK_LOG_CALLS", 0);
  call_function_live_hook_call_log_limit = env_int("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_LIVE_HOOK_CALL_LOG_LIMIT", 8);
  call_function_live_lua_enabled = env_bool("DUNE_CLIENT_PROBE_UE_CALL_FUNCTION_LIVE_LUA_DISPATCH", 0);
...
static uintptr_t configured_process_event_live_hook_target(const char *phase, int *self_test_target) {
  ...
  if (env_bool("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_LIVE_HOOK_SELF_TEST_TARGET", 0)) {
    if (self_test_target) {
      *self_test_target = 1;
    }
    return (uintptr_t)process_event_function_pointer(process_event_self_test_target);
  ...
  uintptr_t target = env_address_auto("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_LIVE_HOOK_ADDRESS", 0);
  if (!target) {
    target = env_address_auto("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_HOOK_ADDRESS", 0);
  }
  if (!target) {
    target = env_address_auto("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_ADDRESS", 0);
  }
...
static void install_process_event_live_hook(const char *phase) {
  if (!env_bool("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_LIVE_HOOK", 0)) {
    return;
  }
  ...
  int call_self_test = env_bool("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_LIVE_HOOK_CALL_SELF_TEST", self_test_target);
  process_event_live_hook_log_calls = env_bool("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_LIVE_HOOK_LOG_CALLS", 0);
  process_event_live_hook_call_log_limit = env_int("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_LIVE_HOOK_CALL_LOG_LIMIT", 8);
  int dispatch_self_test = env_bool("DUNE_CLIENT_PROBE_UE_PROCESS_EVENT_DISPATCH_SELF_TEST", 0);
...
static void run_lua_process_event_self_test(const char *phase) {
  if (!env_bool("DUNE_CLIENT_PROBE_LUA_PROCESS_EVENT_SELF_TEST", 0)) {
    return;
  }
  ...
  const char *script = getenv("DUNE_CLIENT_PROBE_LUA_PROCESS_EVENT_SELF_TEST_SCRIPT");
  if (!script || !script[0]) {
    script =
      "RegisterHook('/Script/DuneProbe.NotTarget:Function', function() return -99 end, function() return -99 end); "
      "return RegisterHook('/Script/DuneProbe.SelfTest:Function', "
      "function(ctx) local p=ctx and ctx.Params; local f=ctx and ctx.Function; local d=f and GetFunctionParams(f); local dm=f and f:GetFunctionParams(); local fdv=f and f:GetParamDescriptor('Value'); local fi=0; if f then f:ForEachParam(function(x) if x and x.IsParm then fi=fi+1 end end) end; local props=d and d.Properties; "
      "local v=p and GetParamDescriptor(p,'Value'); local fv=props and props.Value; local g=p and GetParamDescriptor(p,'Flag'); "
      "local sb=p and GetParamDescriptor(p,'SignedByte'); local md=p and GetParamDescriptor(p,'Mode'); local us=p and GetParamDescriptor(p,'UnsignedShort'); local sl=p and GetParamDescriptor(p,'SignedLarge'); local fl=p and GetParamDescriptor(p,'FloatValue'); local db=p and GetParamDescriptor(p,'DoubleValue'); "
      "local o=p and GetParamDescriptor(p,'TargetObject'); local ov=o and GetParamValue(p,o); "
  ...
  append_log(
    ...
    process_event_self_test_original_calls,
    original_after_hook,
    process_event_self_test_pre_status,
    process_event_self_test_post_status,
    lua_hook_pre_calls,
    lua_hook_post_calls,
    lua_hook_pre_result,
    lua_hook_post_result,
    ...
    params.original_result,
    params.touched,
    final_params.original_result,
    final_params.touched,
...
static size_t ue_layout_slots(void) {
  size_t slots = env_size("DUNE_CLIENT_PROBE_UE_LAYOUT_SLOTS", DEFAULT_UE_LAYOUT_SLOTS);
...
static void validate_ue_anchors(const char *phase) {
  ...
  int layout_probe = env_bool("DUNE_CLIENT_PROBE_UE_LAYOUT_PROBE", 0);
  int uobject_probe = env_bool("DUNE_CLIENT_PROBE_UE_UOBJECT_PROBE", 0);
  int object_array_probe = env_bool("DUNE_CLIENT_PROBE_UE_OBJECT_ARRAY_PROBE", 0);
  int fname_probe = env_bool("DUNE_CLIENT_PROBE_UE_FNAME_PROBE", 0);
  int reflection_probe = env_bool("DUNE_CLIENT_PROBE_UE_REFLECTION_PROBE", 0);
  int reflection_field_walk = env_bool("DUNE_CLIENT_PROBE_UE_REFLECTION_FIELD_WALK", 0);
  int reflection_property_probe = env_bool("DUNE_CLIENT_PROBE_UE_REFLECTION_PROPERTY_PROBE", 0);
  int reflection_value_probe = env_bool("DUNE_CLIENT_PROBE_UE_REFLECTION_VALUE_PROBE", 0);
  ...
  int pointer_probe = env_bool("DUNE_CLIENT_PROBE_UE_POINTER_PROBE", 0) || layout_probe || uobject_probe || reflection_probe;
...
static void run_read_only_scans(const char *phase) {
  if (!env_bool("DUNE_CLIENT_PROBE_SCAN_ENABLED", 0)) {
    return;
  }
  ...
  char exe[4096];
  executable_path(exe, sizeof(exe));
  size_t max_hits = env_size("DUNE_CLIENT_PROBE_SCAN_MAX_HITS_PER_NEEDLE", DEFAULT_MAX_HITS);
  size_t max_mapping_bytes = env_size("DUNE_CLIENT_PROBE_SCAN_MAX_MAPPING_BYTES", DEFAULT_MAX_MAPPING_BYTES);
  ...
  append_log("event=scan-finish phase=%s", phase);
  free(needles);
  free(filters);
  free(patterns);
  free(mappings);
...
static void snapshot(void) {
  int delay = env_int("DUNE_CLIENT_PROBE_SNAPSHOT_DELAY_SECONDS", 0);
  if (delay > 0) {
    append_log("event=snapshot-delay-warning delaySeconds=%d note=constructor_sleep_blocks_process_start", delay);
    sleep((unsigned int)delay);
  }
  append_log("event=snapshot delaySeconds=%d mode=readonly", delay);
...
static void *delayed_ue_probe_thread(void *arg) {
  (void)arg;
  int delay = env_int("DUNE_CLIENT_PROBE_UE_DELAYED_PROBE_SECONDS", 0);
  if (delay <= 0) {
    return NULL;
  }
...
int dune_client_probe_loader_marker(void) {
  return 0x44554e45;
}

__attribute__((constructor)) void dune_client_probe_loader_init(void) {
  char exe[4096];
  executable_path(exe, sizeof(exe));
  if (!exe[0]) {
  ...
  active_process = is_target_process(exe);
  if (!active_process) {
    if (env_bool("DUNE_CLIENT_PROBE_LOG_NON_TARGETS", 0)) {
      append_log("event=skip-non-target exe=%s", exe);
    }
    return;
  }

  append_log("event=loaded exe=%s native=elf note=not-ue4ss-yet", exe);
  {
    const char *delayed_probe_raw = getenv("DUNE_CLIENT_PROBE_UE_DELAYED_PROBE_SECONDS");
    ...
    append_log(
      ...
      env_int("DUNE_CLIENT_PROBE_UE_DELAYED_PROBE_SECONDS", 0)
    );
  }
  log_modules("initial");
...
__attribute__((destructor)) void dune_client_probe_loader_fini(void) {
  if (active_process) {
